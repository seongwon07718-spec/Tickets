...
GUILD_ID = os.getenv("GUILD_ID")  # 비우면 봇이 들어가 있는 모든 서버에 per-guild 동기화
DB_PATH = os.getenv("DB_PATH", "ticketbot.db")  # Zeabur 디스크 쓰면 /data/ticketbot.db 권장
...
# 인텐트: 접두사 명령 안 쓰니까 message_content는 꺼둠(필요 시 True로 바꾸고 포털에서도 켜기)
intents = discord.Intents.default()
intents.members = True
# intents.message_content = True  # 트랜스크립트에 메시지 내용까지 꼭 필요하면 이 줄과 포털 토글을 ON
...
async def on_message(message: discord.Message):
    try:
        await bot.process_commands(message)  # 접두사 명령 안 쓰지만 필수 콜
...
async def on_ready():
    ...
    for grp in (티켓설정, 티켓메시지, 티켓유형):
        try:
            ...
            print(f"[TREE] 그룹 추가 스킵: {getattr(grp,'name','?')} ({e})")
    ...
    # 2) 길드별 싱크(글로벌 → 길드 복사 후 per-guild)
    try:
        if GUILD_ID:
            gid = int(GUILD_ID)
